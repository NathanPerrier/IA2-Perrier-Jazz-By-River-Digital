...
class Place:
    ...
    def __init__(self,state=None, location=None):
        ...
        self.url = weatherAU.PLACES_URL.format(state=state, location=location)
        self.acknowedgment = f'Data courtesy of Bureau of Meteorology ({self.url})'

        req = urllib.request.Request(self.url, data=None, headers={'User-Agent': weatherAU.PLACES_USER_AGENT})
