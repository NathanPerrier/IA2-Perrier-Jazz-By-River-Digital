...
def create_item(request):
    if request.user.groups.filter(name='Vendor').exists():
        if request.method == 'POST':
            try:
                item = FoodAndDrinksItem(name=request.POST['name'].capitalize(), price=float(request.POST['price']), stock=int(request.POST['stock']), description=request.POST['description'], vendor=request.user, image=request.FILES['image'], event=Events.objects.get(id=request.POST['event']))
                item.save()
        ...
        return render(request, 'atc_site//vendor//create_item.html', {'title': 'Create Item', 'user': request.user, 'is_authenticated': request.user.is_authenticated, 'vendor': request.user, 'active_events': Events.objects.filter(date__gte=timezone.now()).all()})
    return render(request, 'atc_site//error.html', {'user': request.user, 'is_authenticated': request.user.is_authenticated, 'error': '400', 'title': 'Forbidden Access', 'desc': 'You do not have permission to access this page. If you believe this is an error, please contact the site administrator.'})
    ...
    if request.user.groups.filter(name='Vendor').exists():
        try:
            ...
            if request.user == item.vendor: 
                ...
                else: return render(request, 'atc_site//vendor//edit_item.html', {'title': 'Edit Item', 'user': request.user, 'is_authenticated': request.user.is_authenticated, 'vendor': request.user, 'item': item, 'active_events': Events.objects.filter(date__gte=timezone.now()).all()})
        except: return render(request, 'atc_site//error.html', {'user': request.user, 'is_authenticated': request.user.is_authenticated, 'error': '403', 'title': 'Bad Request', 'desc': 'An error occured loading this page, if you believe this is an error, please contact the site administrator.'})
    return render(request, 'atc_site//error.html', {'user': request.user, 'is_authenticated': request.user.is_authenticated, 'error': '400', 'title': 'Forbidden Access', 'desc': 'You do not have permission to access this page. If you believe this is an error, please contact the site administrator.'})
